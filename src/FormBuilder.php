...
class FormBuilder
{
    ...
    private function _buildAttrs(array $props = [], array $ignore = []): string
    {
        ...
        if (isset($props['id']) && $props['id'] === false) {
            unset($props['id']);
        } else {
        ...
        if ($this->_type !== 'submit' && $this->_type !== 'anchor') {
            $props['class'] = $props['class'] ?? '';
            $props['class'] .= in_array('class-form-control', $ignore, true) ? '' : " form-control";
        }
    ...
    private function _getHelpText(): string
    {
        ...
        return $this->_help ? '<small id="' . $id . '" class="form-text text-muted">' . $this->_e(
                $this->_help
            ) . '</small>' : '';
    }
    ...
    private function _renderCheckboxOrRadio(): string
    {
        ...
        $attrsHidden = $this->_buildAttrs(['id' => false], ['class-form-control']);
        ...
        <div class="form-group row">
            <div class="col-lg-6 offset-lg-2">
                <div class="custom-control custom-checkbox mr-sm-2">
                    <input type="hidden" value="0" ' . $attrsHidden . '>
                    <input ' . $attrs . '>
                    <label class="custom-control-label text-lg-right" for="' . $for . '">' . $label . '</label>
                </div>
            </div>
        </div>';
    }
     ...
     * @return string|null
     */
    private function _getValidationFieldMessage(
        string $prefix = '<div class="invalid-feedback">', string $suffix = '</div>'
    ) {
